			...
			if (objectToken != null)
			{
				using (ObjectTokenCategory objectTokenCategory = ObjectTokenCategory.Create(category))
				{
					if (objectTokenCategory != null)
					{
						if (objectToken != null)
						{
							using (IEnumerator<ObjectToken> enumerator = objectTokenCategory.GetEnumerator())
							{
						...
						foreach (ObjectToken objectToken3 in objectTokenCategory)
						{
						...
						if (objectToken == null)
						{
							using (IEnumerator<ObjectToken> enumerator3 = objectTokenCategory.GetEnumerator())
							{
		...
		private static bool ParseVersion(string s, ushort[] Version)
		{
			...
			else
			{
				...
				while (num2 < 4 && num < s.Length)
				{
					...
					ushort num4;
					if (!ushort.TryParse(text, ref num4) || num4 > 9999)
					{
						flag = false;
						break;
					}
